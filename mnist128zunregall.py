...
tf.flags.DEFINE_string("logs_dir", "/scratch1/ram095/nips20/logs_mnist128zunregall/", "path to logs directory")
...
def inference(image, keep_prob,z):
    ...
    with tf.variable_scope("inference"):
       ...
       # W_h = utils.weight_variable([1, 7, 7, 4], name="Wh")
        ...
        conv_t1 = utils.conv2d_transpose_strided(fc1, W_t1, b_t1, output_shape=tf.shape(image_net["pool4"]))
...
#    loss = tf.reduce_mean((tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,
 #                                                                         labels=tf.squeeze(annotation, squeeze_dims=[3]),
  #                                                                    name="entropy")))
    ...
    pred_annotation, logits = inference((1-mask)*image, keep_probability,z)
  ...
  #  lossz = 0.1 * tf.reduce_mean(tf.abs(z))
   # loss_all = tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square((image - logits)),[1,2,3])))
    loss_all = tf.reduce_mean(tf.square(image - logits))
    
  #  loss_mask = 0.8*tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square((image - logits)*mask),[1,2,3])))
   # loss_mask = 0.8*tf.reduce_mean(tf.square((image - logits)*mask))
    loss = loss_all
  #  loss = tf.reduce_mean(tf.squared_difference(logits ,annotation ))
    ...
    if FLAGS.mode == "train":
        for itr in xrange(MAX_ITERATION):
            ...
            train_annotations[train_annotations < 127] = 0
            train_images[train_images >= 127] = 255
            train_annotations[train_annotations >= 127] = 255
...
#                print(z_) 
            ...
            if itr % 500 == 0:
                ...
                valid_annotations[valid_annotations < 127] = 0
                valid_images[valid_images < 127] = 0
                valid_annotations[valid_annotations >= 127] = 255
                valid_images[valid_images >= 127] = 255
        ...
        valid_images, valid_annotations = train_dataset_reader.get_random_batch(20)
        valid_annotations[valid_annotations < 127] = 0
        valid_images[valid_images < 127] = 0
        valid_annotations[valid_annotations >= 127] = 255
        valid_images[valid_images >= 127] = 255   
