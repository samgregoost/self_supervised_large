...
import datetime
import BatchDatsetReaderColor as dataset
...
tf.flags.DEFINE_string("logs_dir", "/scratch1/ram095/nips20/logs_colorcocov4/", "path to logs directory")
tf.flags.DEFINE_string("data_dir", "/scratch1/ram095/nips20/datasets/coco", "path to dataset")
...
def inference(images, keep_prob,z,e,is_train):
    ...
    with tf.variable_scope("encoder", reuse = tf.AUTO_REUSE):
        ...
        debn_ = bn
        z_ = z/tf.norm(z)
        bn_ = tf.concat([debn_,z_],axis = 3) + e
    ...
    with tf.variable_scope("decoder", reuse = tf.AUTO_REUSE):
        deconv =  tf.nn.dropout(new_deconv_layer( bn_, [4,4,depth,previousDepth], [FLAGS.batch_size,featureMapSize,featureMapSize,depth], padding='VALID', stride=2, name=("deconv" + str(decoderLayerNum))),keep_prob)
        ...
        a_channel = tf.nn.tanh(new_deconv_layer( debn, [4,4,1,previousDepth], [FLAGS.batch_size,128,128,1], stride=2, name="recon"))
        
        f_map = new_deconv_layer( debn, [4,4,1,previousDepth], [FLAGS.batch_size,128,128,1], stride=2, name="f_map")
        f_bn = tf.nn.relu(batchnorm(f_map, is_train, name=('f_bn'+ str(layer))))
        
        fa_concat = tf.concat([a_channel,f_bn],axis = 3) 
        fa_1 = tf.nn.dropout(new_conv_layer(fa_concat, [4,4,2,2], stride=1, name=("fa_conv1" + str(layer))),keep_prob)
        fa_1bn = tf.nn.relu(batchnorm(fa_1, is_train, name=('fa_1bn'+ str(layer))))

        b_channel = tf.nn.tanh(new_conv_layer(fa_1bn, [4,4,2,1], stride=1, name=("fa_conv2" + str(layer))))
        ...
        print(recon)
        '''
    return a_channel,b_channel, debn_
...
#    concat = tf.reshape(tf.concat([h,z_tiled],axis = 3),[FLAGS.batch_size,-1])
 ...
 #   bn = tf.nn.leaky_relu(batchnorm(fc1, is_train, name=("pred_bn_1")))
  ...
  #  bn2 = tf.nn.leaky_relu(batchnorm(fc2, is_train, name=("pred_bn_2")))
        ...
        fc3 = tf.expand_dims(tf.expand_dims(tf.layers.dense(fc2,50),1),1, name = "pred_fc3")
   # bn2 = tf.nn.leaky_relu(batchnorm(fc1, is_train, name=("pred_bn_2")))
   # z_pred = tf.clip_by_value(tf.nn.tanh(fc1),-0.1,0.1)
 ...
 #   mask2__ = tf.ones([FLAGS.batch_size,78,78,3])
  #  mask2_ = tf.pad(mask2__, [[0,0],[25,25],[25,25],[0,0]])
   # mask2 = mask2_ - mask
    ...
    logits_ae, logits_be,  h_e = inference(image, keep_probability,z,e,istrain)
  ...
  #  loss_mask = 0.8*tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square((image - logits)*mask),[1,2,3])))
    loss_ = tf.reduce_mean(tf.reduce_sum(tf.contrib.layers.flatten(tf.abs((annotation - logits))),1))
    ...
    if FLAGS.mode == "train":
        for itr in xrange(MAX_ITERATION):
          ...
          #  z_ = np.reshape(signal.gaussian(200, std=1),(FLAGS.batch_size,1,1,10))-0.5
            z_ = np.random.uniform(low=-1.0, high=1.0, size=(FLAGS.batch_size,1,1,50))
    ...
    #        train_images[:,x1:w1,y1:h1,:] = 0
          ...
          #  print(train_images)
           ...
           #train_images[:,50:100,50:100,:] =0
            v = 0
           # print(train_images)    
            error_dec =  np.random.normal(0.0,0.001,(FLAGS.batch_size,1,1,4050))
            error_dec_ =  np.random.normal(0.0,0.001,(FLAGS.batch_size,1,1,4050))
