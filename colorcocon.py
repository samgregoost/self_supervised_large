...
tf.flags.DEFINE_string("logs_dir", "/scratch1/ram095/nips20/logs_colorcocon/", "path to logs directory")
...
#    annotation_weights = (tf.exp(tf.abs(annotation))/tf.exp(1.0))
    loss_ = tf.reduce_mean(tf.reduce_sum(tf.contrib.layers.flatten(tf.abs((annotation - logits))    ),1)) + kl_dist_a + kl_dist_b
