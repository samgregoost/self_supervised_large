...
tf.flags.DEFINE_string("logs_dir", "/scratch1/ram095/nips20/logs_deconv224/", "path to logs directory")
...
def inference(image, keep_prob,z):
    ...
    with tf.variable_scope("inference"):
       ...
       # W_h = utils.weight_variable([1, 7, 7, 4], name="Wh")
        ...
        concat1 = tf.concat([tf.reshape(conv8,[-1,7*7*150]),tf.reshape(z,[-1,7*7*10])],axis = 1)
...
def main(argv=None):
    ...
    z = tf.placeholder(tf.float32, shape=[None, 7, 7, 10], name="z")
...
#    loss = tf.reduce_mean((tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,
 #                                                                         labels=tf.squeeze(annotation, squeeze_dims=[3]),
  #                                                                    name="entropy")))
    ...
    mask_ = tf.ones([FLAGS.batch_size,112,112,3])
    mask = tf.pad(mask_, [[0,0],[56,56],[56,56],[0,0]])
  ...
  #  loss = tf.reduce_mean(tf.squared_difference(logits ,annotation ))
    ...
    if FLAGS.mode == "train":
        for itr in xrange(MAX_ITERATION):
            ...
            z_ = np.random.uniform(low=-1.0, high=1.0, size=(FLAGS.batch_size,7,7,10))
       ...
       # pred = np.squeeze(pred, axis=3)
        ...
        for itr in range(FLAGS.batch_size):
           ...
           # utils.save_image(pred[itr].astype(np.uint8), FLAGS.logs_dir, name="predz_" + str(5+itr))
            utils.save_image(valid_images_masked[itr], FLAGS.logs_dir, name="inp_" + str(5+itr))
            utils.save_image(valid_annotations[itr], FLAGS.logs_dir, name="gt_" + str(5+itr))
            utils.save_image(pred[itr], FLAGS.logs_dir, name="predz_" + str(5+itr))
