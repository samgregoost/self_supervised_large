...
tf.flags.DEFINE_string("logs_dir", "/scratch1/ram095/nips20/logs_mnist128zunreg/", "path to logs directory")
...
def inference(image, keep_prob,z):
    ...
    with tf.variable_scope("inference"):
       ...
       # W_h = utils.weight_variable([1, 7, 7, 4], name="Wh")
        ...
        concat1 = tf.reshape(tf.concat([conv8, z],axis = 3),[-1,4*4*278])
        
        fc1 = tf.reshape(tf.layers.dense(concat1,4*4*278,activation = tf.nn.relu),[-1,4,4,278])
        ...
        conv_t1 = tf.nn.relu(utils.conv2d_transpose_strided(fc1, W_t1, b_t1, output_shape=tf.shape(image_net["pool4"])))
        fuse_1 = tf.add(conv_t1, image_net["pool4"], name="fuse_1")

        deconv_shape2 = image_net["pool3"].get_shape()
        W_t2 = utils.weight_variable([4, 4, deconv_shape2[3].value, deconv_shape1[3].value], name="W_t2")
        b_t2 = utils.bias_variable([deconv_shape2[3].value], name="b_t2")
        conv_t2 = tf.nn.relu(utils.conv2d_transpose_strided(fuse_1, W_t2, b_t2, output_shape=tf.shape(image_net["pool3"])))
        fuse_2 = tf.add(conv_t2, image_net["pool3"], name="fuse_2")

        shape = tf.shape(image)
        deconv_shape3 = tf.stack([shape[0], shape[1], shape[2], 1])
        W_t3 = utils.weight_variable([16, 16, 1, deconv_shape2[3].value], name="W_t3")
        b_t3 = utils.bias_variable([1], name="b_t3")
        conv_t3 = tf.nn.sigmoid(utils.conv2d_transpose_strided(fuse_2, W_t3, b_t3, output_shape=deconv_shape3, stride=8))
 ...
 #   mask2__ = tf.ones([FLAGS.batch_size,78,78,3])
  #  mask2_ = tf.pad(mask2__, [[0,0],[25,25],[25,25],[0,0]])
   # mask2 = mask2_ - mask
    ...
    pred_annotation, logits = inference((1-mask)*image + mask*0.0, keep_probability,z)
  ...
  #  loss = tf.reduce_mean(tf.squared_difference(logits ,annotation ))
    ...
    if FLAGS.mode == "train":
        for itr in xrange(MAX_ITERATION):
            ...
            print(np.min(train_annotations))
...
#                print(z_)
       ...
       # pred = np.squeeze(pred, axis=3)
        valid_images_masked = (valid_images_masked + 1.)/2. *255.0
        pred = (pred+1.)/2. * 255
        valid_annotations = (valid_annotations+1.)/2. *255.0
