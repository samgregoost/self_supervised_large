...
tf.flags.DEFINE_string("logs_dir", "/scratch1/ram095/nips20/logs_contextrr/", "path to logs directory")
 ...
 #   mask2__ = tf.ones([FLAGS.batch_size,78,78,3])
  #  mask2_ = tf.pad(mask2__, [[0,0],[25,25],[25,25],[0,0]])
   # mask2 = mask2_ - mask
    ...
    logits = inference((1-mask)*image + mask*0.0, keep_probability,z,istrain)
  ...
  #  loss_mask = 0.8*tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square((image - logits)*mask),[1,2,3])))
    loss_mask = tf.reduce_mean(tf.reduce_sum(tf.contrib.layers.flatten(tf.abs((image - logits)*mask)),1))
    loss =  loss_mask
        ...
        for itr in xrange(MAX_ITERATION):
          ...
          #  print(train_images)
            ...
            if cond <=4:
                ...
                train_images_m[train_images_m < 0.] = random.uniform(-0.4, 0.0)
                train_annotations_m[train_annotations_m < 0.] = random.uniform(-0.4, 0.0)
                train_images_m[train_images_m >= 0.] = random.uniform(0.0, 0.2)
                train_annotations_m[train_annotations_m >= 0.] = random.uniform(0.0, 0.2)
           ...
           # utils.save_image(pred[itr].astype(np.uint8), FLAGS.logs_dir, name="predz_" + str(5+itr))
                #    utils.save_image(train_images[itr_].astype(np.uint8), FLAGS.logs_dir, name="inp_" + str(5+itr_))
...
#                print(z_) 
               ...
               # print(v.shape)
               # print(z_.shape)
            feed_dict = {image: train_images, annotation: train_annotations, keep_probability: 0.85,mask:random_mask(64), z: z_, istrain:True }
      ...
      #  valid_images[:,x1:w1,y1:h1,:] = 0
        ...
        z_ = np.random.uniform(low=-1.0, high=1.0, size=(FLAGS.batch_size,4,4,10))
        feed_dict = {image: valid_images, annotation: valid_annotations, keep_probability: 1.0, z: z_, istrain:False,mask:random_mask(64) }
        v= 0
        for p in range(0):
                z_ol = np.copy(z_)
               # print("666666666666666666666666666666666666666")
