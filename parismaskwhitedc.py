...
tf.flags.DEFINE_string("logs_dir", "/scratch1/ram095/nips20/logs_parisfc/", "path to logs directory")
...
def inference(image, keep_prob,z):
    ...
    with tf.variable_scope("inference"):
       ...
       # W_h = utils.weight_variable([1, 7, 7, 4], name="Wh")
        ...
        W_t1 = utils.weight_variable([4, 4, deconv_shape1[3].value, 200], name="W_t1")
...
def main(argv=None):
    ...
    z = tf.placeholder(tf.float32, shape=[None, 4, 4, 50], name="z")
   ...
   # loss = tf.reduce_mean(tf.squared_difference(logits ,annotation ))
    ...
    if FLAGS.mode == "train":
        for itr in xrange(MAX_ITERATION):
            ...
            z_ = np.random.uniform(low=-1.0, high=1.0, size=(FLAGS.batch_size,4,4,50))
         #   print(train_images)
