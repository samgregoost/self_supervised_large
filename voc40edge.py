...
tf.flags.DEFINE_string("logs_dir", "/scratch1/ram095/nips20/logs_vocedge40/", "path to logs directory")
tf.flags.DEFINE_string("data_dir", "/scratch1/ram095/nips20/datasets/pascal", "path to dataset")
...
#    loss = tf.reduce_mean((tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,
 ...
 #   mask2__ = tf.ones([FLAGS.batch_size,78,78,3])
  ...
  #  loss_mask = 0.8*tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square((image - logits)*mask),[1,2,3])))
    ...
    loss_ =  0.2*loss_mask + loss_mask2
