...
tf.flags.DEFINE_string("logs_dir", "/scratch1/ram095/nips20/logs_colorcococa/", "path to logs directory")
...
def inference(images, keep_prob,z,e,is_train):
    ...
    encoderLayerNum = int(math.log(IMAGE_SIZE) / math.log(2)) #6
    encoderLayerNum = encoderLayerNum - 1 #5   minus 1 because the second last layer directly go from 4x4 to 1x1 
    print("encoderLayerNum=", encoderLayerNum)
    encoderLayerNum = encoderLayerNum

    decoderLayerNum = int(math.log(IMAGE_SIZE) / math.log(2)) #6
    decoderLayerNum = decoderLayerNum - 1 # 5
    ...
    with tf.variable_scope("encoder", reuse = tf.AUTO_REUSE):
        ...
        '''
        for layer in range(1, encoderLayerNum): #1 - 4
    ...
    with tf.variable_scope("decoder", reuse = tf.AUTO_REUSE):
        ...
        deconv4 = new_deconv_layer( debn5, [4,4,depth,previousDepth], [FLAGS.batch_size,featureMapSize,featureMapSize,depth], stride=2, name=("deconv4")) + conv3
        debn4 = tf.nn.relu(batchnorm(deconv4, is_train, name=('debn4')))
        previousDepth = depth
        depth = depth / 2
        featureMapSize = featureMapSize *2

        deconv3 = new_deconv_layer( debn4, [4,4,depth,previousDepth], [FLAGS.batch_size,featureMapSize,featureMapSize,depth], stride=2, name=("deconv3")) + conv2
        ...
        deconv2 = new_deconv_layer( debn3, [4,4,depth,previousDepth], [FLAGS.batch_size,featureMapSize,featureMapSize,depth], stride=2, name=("deconv2")) + conv1
        ...
        for layer in range(decoderLayerNum-1,1, -1):
            ...
            featureMapSize = featureMapSize *2
        '''
        recon = tf.nn.tanh(new_deconv_layer( debn2, [4,4,2,previousDepth], [FLAGS.batch_size,64,64,2], stride=2, name="recon"))
