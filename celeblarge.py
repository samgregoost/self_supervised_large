...
tf.flags.DEFINE_string("logs_dir", "/flush5/ram095/nips20/logs_celeblarge/", "path to logs directory")
tf.flags.DEFINE_string("data_dir", "/flush5/ram095/nips20/datasets/CelebAMask-HQ", "path to dataset")
...
def random_mask(input_size):
    ...
    x1 =  32 #40 # 32 # random.randint(5, 20)
    w1 = 64 # 48# 64 #random.randint(20, 34)
    y1 =32 # # 32 #random.randint(5, 20)
    h1 =64 # 64 #random.randint(20, 34)

    mask = np.zeros((1,128,128,1))
    mask[:,x1:x1+w1,y1:y1+h1,:] = 1.0

    mask2 = np.zeros((1,128,128,1))
    mask2[:,x1-10:x1+w1+10,y1-10:h1+y1+10,:] = 1.0
...
#    loss = tf.reduce_mean((tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,
 ...
 #   mask2__ = tf.ones([FLAGS.batch_size,78,78,3])
  ...
  #  loss_mask = 0.8*tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square((image - logits)*mask),[1,2,3])))
    ...
    loss_ = loss_mask + loss_mask2 +  loss_ssim
  ...
  #  loss = tf.reduce_mean(tf.squared_difference(logits ,annotation ))
   ...
   # zloss = tf.reduce_mean(tf.losses.cosine_distance(tf.contrib.layers.flatten(z_new) ,tf.contrib.layers.flatten(z_pred),axis =1)) 
    zloss_ = 0.001*tf.reduce_mean(tf.reduce_sum(tf.contrib.layers.flatten(tf.abs((z_pred - z_new))),1))
...
#    z_loss = zloss_ + 0.1* zloss_lip# + zloss_lip_inv
    ...
    lip_loss_pred = 0.001 *tf.reduce_mean(tf.reduce_sum(tf.contrib.layers.flatten(tf.abs((z_pred - z_pred_e))),1))     
...
#                print(z_) 
     ...
     #           valid_images[:,x1:w1,y1:h1,:] = 0
                ...
                saver.save(sess, FLAGS.logs_dir + "model_big.ckpt", save_itr)
...
#        pred = pred + 1./2.0*255
       ...
       # for itr in range(FLAGS.batch_size):
        ...
        for itr in range(FLAGS.batch_size):
            utils.save_image(valid_images_masked[itr].astype(np.uint8), FLAGS.logs_dir, name="out" + str(5+itr)+"c" )
            utils.save_image(pred[itr].astype(np.uint8), FLAGS.logs_dir, name="out" + str(5+itr)+ "a")     
            utils.save_image(valid_annotations_[itr].astype(np.uint8), FLAGS.logs_dir, name="out" + str(5+itr) + "b" )        
