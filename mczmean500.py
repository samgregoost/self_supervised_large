...
tf.flags.DEFINE_string("logs_dir", "/scratch1/ram095/nips20/logs_mczmean500/", "path to logs directory")
...
def inference_mse(images, keep_prob,is_train):
    ...
    with tf.variable_scope("inference_mse"):
        ...
        debn = tf.nn.relu(batchnorm(deconv, is_train, name=("debn" + str(decoderLayerNum))))
...
def predictor(h,z, is_train):
 #   z_tiled = tf.tile(z,[1,4,4,1])
    concat = tf.concat([tf.contrib.layers.flatten(h),tf.contrib.layers.flatten(z)],axis = 1)
#    concat = tf.reshape(tf.concat([h,z_tiled],axis = 3),[FLAGS.batch_size,-1])
    fc1 = tf.nn.leaky_relu(tf.layers.dense(concat,512))
 #   bn = tf.nn.leaky_relu(batchnorm(fc1, is_train, name=("pred_bn_1")))
   
    fc2 = tf.nn.leaky_relu(tf.layers.dense(fc1,512))
  #  bn2 = tf.nn.leaky_relu(batchnorm(fc2, is_train, name=("pred_bn_2")))

    fc3 = tf.expand_dims(tf.expand_dims(tf.layers.dense(fc2,512),1),1)
   # bn2 = tf.nn.leaky_relu(batchnorm(fc1, is_train, name=("pred_bn_2")))
   # z_pred = tf.clip_by_value(tf.nn.tanh(fc1),-0.1,0.1)
...
def main(argv=None):
    ...
    z = tf.placeholder(tf.float32, shape=[None, 1, 1, 512], name="z")
    mask = tf.placeholder(tf.float32, shape=[None, 64, 64, 1], name="mask")
    z_new = tf.placeholder(tf.float32, shape=[None, 1, 1, 512], name="z_new")
 ...
 #   mask2__ = tf.ones([FLAGS.batch_size,78,78,3])
  #  mask2_ = tf.pad(mask2__, [[0,0],[25,25],[25,25],[0,0]])
   # mask2 = mask2_ - mask
    ...
    logits_mse = inference_mse((1-mask)*image + mask*0.0, keep_probability,istrain)
  ...
  #  loss_mask = 0.8*tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square((image - logits)*mask),[1,2,3])))
    ...
    loss_mse = tf.reduce_mean(tf.reduce_sum(tf.contrib.layers.flatten(tf.abs((annotation - logits_mse)*mask)),1))
  #  loss = tf.reduce_mean(tf.squared_difference(logits ,annotation ))
    loss_summary = tf.summary.scalar("entropy", loss)
    loss_summary_mse = tf.summary.scalar("entropy_mse", loss_mse)
   # zloss = tf.reduce_mean(tf.losses.cosine_distance(tf.contrib.layers.flatten(z_new) ,tf.contrib.layers.flatten(z_pred),axis =1)) 
    zloss = tf.reduce_mean(tf.reduce_sum(tf.contrib.layers.flatten(tf.abs((z_pred - z_new))),1))
    
    init_loss = tf.reduce_mean(tf.reduce_sum(tf.contrib.layers.flatten(tf.abs((logits_mse-logits)*mask)),1))

    grads = train_z(loss_mask,z)    
    grads_init = train_z(init_loss,z)
    ...
    train_mse = train(loss_mse, trainable_var)
    train_init = train(init_loss,trainable_var)
    ...
    if FLAGS.mode == 'train' or FLAGS.mode == 'train_mse':
    ...
    if FLAGS.mode == "train" or FLAGS.mode == "train_mse":
        for itr in xrange(MAX_ITERATION):
            ...
            z_ = np.random.uniform(low=-1.0, high=1.0, size=(FLAGS.batch_size,1,1,512))
           ...
           # print(train_images)    
            if FLAGS.mode == "train":
               ...
               # print("666666666666666666666666666666666666666")
                    feed_dict = {image: train_images, annotation: train_annotations, keep_probability: 0.85, z: z_,mask:r_m, istrain:True }
                    z_loss, summ = sess.run([init_loss,loss_summary], feed_dict=feed_dict)
                    print("Step: %d, z_step: %d, Init_loss:%g" % (itr,p,z_loss))
#                print(z_)
                    g = sess.run([grads_init],feed_dict=feed_dict)
                    v_prev = np.copy(v)
               # print(g[0][0].shape)
                    v = 0.001*v - 0.1*g[0][0]
...
#                print(z_) 
               ...
               # print(g[0][0].shape)
                    ...
                    z_ol_interp = m(z_ol)
                    z_interp = m(z_)
                    _,z_pred_loss =sess.run([train_pred,zloss],feed_dict={image: train_images,mask:r_m,z:z_ol_interp,z_new:z_interp,istrain:True,keep_probability: 0.85})
                    print("Step: %d, z_step: %d, z_pred_loss:%g" % (itr,p,z_pred_loss))
                    '''
            ...
            elif FLAGS.mode == "train_mse":
                feed_dict = {image: train_images, annotation: train_annotations, keep_probability:0.85,mask:r_m, istrain:True }
                sess.run(train_mse, feed_dict=feed_dict)
            ...
            if itr % 10 == 0:
                ...
                if FLAGS.mode == "train":
                    ...
                    train_loss, summary_str = sess.run([loss_mse, loss_summary_mse], feed_dict=feed_dict)
     ...
     #           valid_images[:,x1:w1,y1:h1,:] = 0
                if FLAGS.mode == "train": 
                    ...
                    valid_loss, summary_sva = sess.run([loss_mse, loss_summary_mse], feed_dict={image: valid_images,mask:random_mask(64), annotation: valid_annotations,
                                                       keep_probability: 1.0, istrain:False })
  ...
  #      feed_dict = {image: valid_images, annotation: valid_annotations, keep_probability: 0.85, z: z_, istrain:False,mask:r_m }
        for p in range(0):
                    z_ol = np.copy(z_)
               # print("666666666666666666666666666666666666666")
                    feed_dict = {image: valid_images, annotation: valid_annotations, keep_probability: 0.85, z: z_,mask:r_m, istrain:True }
                    z_loss, summ = sess.run([init_loss,loss_summary], feed_dict=feed_dict)
                    print("z_step: %d, Init_loss:%g" % (p,z_loss))
#                print(z_)
                    g = sess.run([grads_init],feed_dict=feed_dict)
                    v_prev = np.copy(v)
               # print(g[0][0].shape)
...
#                print(z_)
              ...
              #  z_ = z_ol + 0.001 * v_prev + (1+0.001)*v
               ...
               # print(z_)
                ...
                #_ = sess.run(z_pred,feed_dict = {image: valid_images, annotation: valid_annotations, keep_probability: 0.85, z:z_ol, istrain:False,mask:r_m})
              # z_ = np.clip(z_, -1.0, 1.0)
               # print(z_pred_loss)
