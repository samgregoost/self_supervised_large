...
tf.flags.DEFINE_string("logs_dir", "/scratch1/ram095/nips20/logs_colorcocow/", "path to logs directory")
...
#    loss = tf.reduce_mean((tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,
 ...
 #   mask2__ = tf.ones([FLAGS.batch_size,78,78,3])
  ...
  #  loss_mask = 0.8*tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square((image - logits)*mask),[1,2,3])))
    ...
    annotation_weights = (tf.exp(tf.abs(annotation))/tf.exp(1.0))/annotation_weights_norm 
    loss_ = tf.reduce_mean(tf.reduce_sum(tf.contrib.layers.flatten(tf.abs((annotation - logits))*annotation_weights    ),1))
