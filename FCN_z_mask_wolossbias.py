...
tf.flags.DEFINE_string("logs_dir", "/scratch1/ram095/nips20/logs_mask_wolb", "path to logs directory")
...
def main(argv=None):
    ...
    z = tf.placeholder(tf.float32, shape=[None, 4, 4, 3], name="z")
...
#    loss = tf.reduce_mean((tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,
 #                                                                         labels=tf.squeeze(annotation, squeeze_dims=[3]),
  #                                                                    name="entropy")))
    ...
    loss1 = tf.reduce_mean(tf.square((image - logits)*mask))
   ...
   # loss = tf.reduce_mean(tf.squared_difference(logits ,annotation ))
    ...
    if FLAGS.mode == "train":
        for itr in xrange(MAX_ITERATION):
            ...
            z_ = np.random.uniform(low=-1.0, high=1.0, size=(FLAGS.batch_size,4,4,3))
