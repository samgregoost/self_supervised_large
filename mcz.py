...
tf.flags.DEFINE_string("logs_dir", "/scratch1/ram095/nips20/logs_mcz/", "path to logs directory")
  ...
  #  loss = tf.reduce_mean(tf.squared_difference(logits ,annotation ))
   ...
   # zloss = tf.reduce_mean(tf.losses.cosine_distance(tf.contrib.layers.flatten(z_new) ,tf.contrib.layers.flatten(z_pred),axis =1)) 
    ...
    #        train_images[:,x1:w1,y1:h1,:] = 0
          ...
          #  print(train_images)
            r_m = random_mask(64)
            feed_dict = {image: train_images, annotation: train_annotations, keep_probability: 1.0, z: z_,mask:r_m, istrain:True }
...
#                print(z_) 
              ...
              #  z_ = np.clip(z_, -1.0, 1.0)
                _,z_pred_loss =sess.run([train_pred,zloss],feed_dict={image: train_images,mask:r_m,z:z_ol,z_new:z_,istrain:True,keep_probability: 1.0})
                print("Step: %d, z_step: %d, z_pred_loss:%g" % (itr,p,z_pred_loss))
               # z_ = np.clip(z_, -1.0, 1.0)
               # print(v.shape)
               # print(z_.shape)
            feed_dict = {image: train_images, annotation: train_annotations, keep_probability: 1.0,mask:r_m, z: z_, istrain:True }
     ...
     #           valid_images[:,x1:w1,y1:h1,:] = 0
      ...
      #  valid_images[:,x1:w1,y1:h1,:] = 0
        r_m = random_mask(64)      
        z_ = np.random.uniform(low=-1.0, high=1.0, size=(FLAGS.batch_size,1,1,10))
        feed_dict = {image: valid_images, annotation: valid_annotations, keep_probability: 1.0, z: z_, istrain:False,mask:r_m }
        v= 0
