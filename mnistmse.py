...
tf.flags.DEFINE_string("logs_dir", "/scratch1/ram095/nips20/logs_mnistmse/", "path to logs directory")
...
def inference(image, keep_prob):
    ...
    #    fc1 = tf.reshape(tf.layers.dense(concat1,4*4*278,activation = tf.nn.relu),[-1,4,4,278])
        ...
        W_t1 = utils.weight_variable([4, 4, deconv_shape1[3].value, 150], name="W_t1")
...
#    loss = tf.reduce_mean((tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,
 #                                                                         labels=tf.squeeze(annotation, squeeze_dims=[3]),
  #                                                                    name="entropy")))
    ...
    pred_annotation, logits = inference((1-mask)*image + mask*255, keep_probability)
