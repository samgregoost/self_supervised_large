...
tf.flags.DEFINE_string("logs_dir", "/flush5/ram095/nips20/logs_falarge/", "path to logs directory")
tf.flags.DEFINE_string("data_dir", "/flush5/ram095/nips20/datasets/fa", "path to dataset")
...
EPOCHS_COMPLETED = 0
FILE_LIST = []
FILE_GLOB = ('/flush5/ram095/nips20/datasets/irregular_mask/disocclusion_img_mask/*.png')
...
def inference_(images, keep_prob,z,e,is_train):
    """
    Semantic segmentation network definition
    :param image: input image. Should have values in range 0-255
    :param keep_prob:
    ...
    with tf.variable_scope("encoder", reuse = tf.AUTO_REUSE):
      ...
      #  bn4 = tf.nn.leaky_relu(batchnorm(conv4, is_train, name=("bn4")))
        ...
        depth = depth * 2
            ...
            # last layer
        conv5 = new_conv_layer(bn4, [4,4,previousDepth,4000], stride=2, padding='VALID', name=('conv5'))
      #  bn5 = tf.nn.leaky_relu(batchnorm(conv5, is_train, name=("bn5")))
        ...
        deconv5 =  tf.nn.dropout(new_deconv_layer( bn5, [4,4,depth,previousDepth], [FLAGS.batch_size,featureMapSize,featureMapSize,depth], padding='VALID', stride=2, name=("deconv5")),keep_prob)

        #debn_ = tf.nn.relu(batchnorm(deconv, is_train, name=("debn" + str(decoderLayerNum))))
        z_ = z/tf.norm(z)
      #  debn_ = tf.nn.relu(batchnorm(deconv5, is_train, name=("debn5")))
        debn_= tf.nn.relu(deconv5)
        debn5 = tf.concat([debn_,tf.tile(z_,[1,4,4,1])],axis = 3) + e
    ...
    with tf.variable_scope("decoder", reuse = tf.AUTO_REUSE):
      ...
      #  debn3 = tf.nn.relu(batchnorm(deconv3, is_train, name=('debn3'))) + bn2
       ...
       # debn2 = tf.nn.relu(batchnorm(deconv2, is_train, name=('debn2'))) + bn1
        ...
        recon = tf.nn.tanh(new_deconv_layer( debn2, [4,4,3,previousDepth], [FLAGS.batch_size,64,64,3], stride=2, name="recon"))
...
def next_mask_list():
        ...
        start = BATCH_OFFSET
        print(start)
        BATCH_OFFSET += FLAGS.batch_size
        if BATCH_OFFSET > len(FILE_LIST):
            # Finished epoch
            EPOCHS_COMPLETED += 1
            print("****************** Epochs completed for mask: " + str(EPOCHS_COMPLETED) + "******************")
            # Shuffle the data
...
def random_mask(input_size):
    ...
    mask_file_list = next_mask_list()
    mask_array = np.empty((0,128, 128))
    for mf in mask_file_list:
        mask = misc.imread(mf, flatten  = True)
        mask = misc.imresize(mask,[IMAGE_SIZE, IMAGE_SIZE], interp = 'nearest')/255.0
  #      print(np.max(mask))
 #       print(np.min(mask))
       # print(mask)
        mask = np.where(mask > 0.6, 0.0, 1.0)
        mask = ndimage.binary_dilation(mask)
        mask = np.reshape(np.where(mask > 0.5, 1.0, 0.0),(1,128,128))
        print(np.count_nonzero(mask)/(128*128) )
 ...
 #   mask_array = np.vstack(mask_list)
    ...
    mask_array = np.reshape(mask_array,(20,128,128,1))
    '''
    x1 = 32 # random.randint(5, 20)
    w1 = 32 #random.randint(20, 34)
    y1 = 32 #random.randint(5, 20)
    h1 = 32 #random.randint(20, 34)
...
def main(argv=None):
    ...
    mask = tf.placeholder(tf.float32, shape=[None, IMAGE_SIZE, IMAGE_SIZE, 1], name="mask")
    mask2 = tf.placeholder(tf.float32, shape=[None, IMAGE_SIZE, IMAGE_SIZE, 1], name="mask2")
   ...
   #z_lip_inv = tf.placeholder(tf.float32, shape=[None, 1, 1, 10], name="z_lip_inv")
    ...
    image_ = tf.image.resize_with_crop_or_pad(image_,150,150 )
    image_ = tf.image.random_crop(image_, size = (FLAGS.batch_size, 128,128,3))
    ...
    mask_ = tf.image.random_flip_up_down(mask)
    mask_ = tf.image.random_flip_left_right(mask_)
  #  mask_ = tf.image.resize_with_crop_or_pad(mask_,80,80 )
  # mask_ = tf.image.random_crop(mask_, size = (FLAGS.batch_size, 64,64,1))
    mask_ = tf.image.rot90(mask_, k = tf.random_uniform(shape=[], minval=0, maxval=4, dtype=tf.int32))
 ...
 #   mask2__ = tf.ones([FLAGS.batch_size,78,78,3])
  #  mask2_ = tf.pad(mask2__, [[0,0],[25,25],[25,25],[0,0]])
   # mask2 = mask2_ - mask
    ...
    logits, h  = inference((1-mask)*image + mask*-2.0,keep_probability,z,0.0,istrain)
    logits_e, h_e = inference((1-mask)*image + mask*-2.0, keep_probability,z,e,istrain)
  ...
  #  loss_mask = 0.8*tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square((image - logits)*mask),[1,2,3])))
    loss_mask =0.001* tf.reduce_mean(tf.reduce_sum(tf.contrib.layers.flatten(tf.abs((annotation - logits)*mask)),1))
    loss_mask2 = 0.001* tf.reduce_mean(tf.reduce_sum(tf.contrib.layers.flatten(tf.abs((annotation - logits)*mask2)),1)) 
    loss_ssim =  -1.0*tf.log(tf.reduce_mean(tf.image.ssim(annotation_ssim , logits_ssim ,max_val = 255.0)))
    loss_ = loss_mask +  loss_ssim


  #  loss_ = 0.5* loss_mask + loss_mask2
...
#    z_loss = zloss_ + 0.1* zloss_lip# + zloss_lip_inv
    ...
    lip_loss_dec =0.001* tf.reduce_mean(tf.reduce_sum(tf.contrib.layers.flatten(tf.abs((logits - logits_e))),1))
    loss = loss_ + 0.1*lip_loss_dec 
   
    lip_loss_pred = tf.reduce_mean(tf.reduce_sum(tf.contrib.layers.flatten(tf.abs((z_pred - z_pred_e))),1))     
    zloss = zloss_ + 0.5*lip_loss_pred
    ...
    if FLAGS.mode == "train":
        for itr in xrange(MAX_ITERATION):
            ...
            train_images_, train_annotations_ = train_dataset_reader.next_batch(FLAGS.batch_size)
            train_images = sess.run(image_, feed_dict={image:train_images_})
            train_annotations = np.copy(train_images)

            print(np.max(train_images))
            r_m_, r_m2_ = random_mask(64)
            r_m, r_m2 =  sess.run([mask_, mask2_], feed_dict={mask:r_m_, mask2:r_m2_  })
           ...
           # print("Step: %d, lip_loss:%g" % (itr,lloss))
            ...
            #    z_l = z_ol + np.random.normal(0.0,0.001)
               ...
               # print("Step: %d, z_step: %d, Train_loss:%g" % (itr,p,z_loss))
                z_loss,l_mask,l_ssim, l_lip, summ = sess.run([loss, loss_mask, loss_ssim, lip_loss_dec, loss_summary], feed_dict=feed_dict)
                print("Step: %d, z_step: %d, Train_loss:%g, Loss_mask:%g, SSIM_loss:%g, lip_loss:%g, " % (itr,p,z_loss,l_mask,l_ssim, l_lip))
#                print(z_) 
...
#                print(z_)
       ...
       # z_ = m_(z_)
        ...
        valid_images_masked = ((1-r_m)*valid_images + r_m*1.0 + 1.)/2.0*255
...
#        pred = pred + 1./2.0*255
       ...
       # for itr in range(FLAGS.batch_size):
        ...
        for itr in range(FLAGS.batch_size):
            utils.save_image(valid_images_masked[itr].astype(np.uint8), FLAGS.logs_dir, name="out" + str(85+itr) + "c" )
            utils.save_image(pred[itr].astype(np.uint8), FLAGS.logs_dir, name="out" + str(85+itr)+ "a" )     
            utils.save_image(valid_annotations_[itr].astype(np.uint8), FLAGS.logs_dir, name="out" + str(85+itr) + "b" )        
