...
tf.flags.DEFINE_string("logs_dir", "/scratch1/ram095/nips20/logs_cfaredge/", "path to logs directory")
tf.flags.DEFINE_string("data_dir", "/scratch1/ram095/nips20/datasets/CIFAR/", "path to dataset")
...
def random_mask(input_size):
    x1 =16#  random.randint(5, 7)
    w1 =48 #random.randint(45, 50)
    y1 =16# random.randint(5, 7)
    h1 =48# random.randint(20, 34)

    mask = np.zeros((1,64,64,1))
    mask[:,x1:w1,y1:h1,:] = 1.0
    mask2 = np.zeros((1,64,64,1))
    mask2[:,x1-5:w1+5,y1-5:h1+5,:] = 1.0
    mask2 = mask2 - mask
...
def main(argv=None):
    ...
    mask2 = tf.placeholder(tf.float32, shape=[None, 64, 64, 1], name="mask")
...
#                print(z_) 
               ...
               # print(v.shape)
               # print(z_.shape)
            feed_dict = {image: train_images, annotation: train_annotations, keep_probability:0.85,mask:r_m,e:error_dec,mask2:r_m2, z: z_, istrain:True }
     ...
     #           valid_images[:,x1:w1,y1:h1,:] = 0
                ...
                valid_loss, summary_sva = sess.run([loss, loss_summary], feed_dict={image: valid_images,mask:r_m,mask2:r_m2, annotation: valid_annotations,
...
#        pred = pred + 1./2.0*255
       ...
       # for itr in range(FLAGS.batch_size):
        ...
        for itr in range(FLAGS.batch_size):
            utils.save_image(misc.imresize(valid_images_masked[itr].astype(np.uint8),50), FLAGS.logs_dir, name="inp_" + str(5+itr) )
            utils.save_image(misc.imresize(pred[itr].astype(np.uint8),50), FLAGS.logs_dir, name="predz_" + str(5+itr) )     
            utils.save_image(misc.imresize(valid_annotations_[itr].astype(np.uint8),50), FLAGS.logs_dir, name="gt_" + str(5+itr) )        
