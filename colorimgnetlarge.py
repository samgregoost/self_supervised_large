...
import read_MITSceneParsingDataImage as scene_parsing
import datetime
...
tf.flags.DEFINE_string("logs_dir", "/scratch1/ram095/nips20/logs_colorimglarge/", "path to logs directory")
tf.flags.DEFINE_string("data_dir", "/image/datasets/ml/imagenet", "path to dataset")
...
#        pred = pred + 1./2.0*255
        print(np.max(pred_))
        print(np.min(pred_))
        pred = np.reshape(np.concatenate((valid_images,pred_), axis=3),(-1,128,128,3))

        valid_annotations_ = valid_annotations * 128.0
        valid_annotations = np.reshape(np.concatenate((valid_images,  valid_annotations_), axis=3),(-1,128,128,3))
       ...
       # for itr in range(FLAGS.batch_size):
        ...
        for itr in range(FLAGS.batch_size):
            utils.save_image(valid_images_gray[itr], FLAGS.logs_dir, name="out" + str(itr) + "a")
            utils.save_image(color.lab2rgb(pred[itr]), FLAGS.logs_dir, name="out" + str(itr) + "b" )
            utils.save_image(color.lab2rgb(valid_annotations[itr]), FLAGS.logs_dir, name="out" + str(itr) + "c" )
