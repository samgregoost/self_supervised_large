...
tf.flags.DEFINE_string("logs_dir", "/scratch1/ram095/nips20/logs_context/", "path to logs directory")
...
def inference(images, keep_prob,z,is_train):
    ...
    with tf.variable_scope("inference"):
        ...
        deconv = new_deconv_layer( bn, [4,4,depth,previousDepth], [FLAGS.batch_size,featureMapSize,featureMapSize,depth], padding='VALID', stride=2, name=("deconv" + str(decoderLayerNum)))
        ...
        for layer in range(decoderLayerNum-1,1, -1):
            ...
            deconv =  tf.nn.dropout(new_deconv_layer( previousFeatureMap, [4,4,depth,previousDepth], [FLAGS.batch_size,featureMapSize,featureMapSize,depth], stride=2, name=("deconv" + str(layer))),keep_prob)
  ...
  #  loss = tf.reduce_mean(tf.squared_difference(logits ,annotation ))
    ...
    if FLAGS.mode == "train":
        for itr in xrange(MAX_ITERATION):
          ...
          #  print(train_images)
            feed_dict = {image: train_images, annotation: train_annotations, keep_probability: 0.85, z: z_, istrain:True }
...
#                print(z_) 
        ...
        z_ = np.random.uniform(low=-1.0, high=1.0, size=(FLAGS.batch_size,4,4,10))
        feed_dict = {image: valid_images, annotation: valid_annotations, keep_probability: 1.0, z: z_, istrain:False }
        v= 0
        for p in range(0):
                z_ol = np.copy(z_)
               # print("666666666666666666666666666666666666666")
