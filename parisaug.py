...
tf.flags.DEFINE_string("logs_dir", "/flush5/ram095/nips20/logs_parisaug/", "path to logs directory")
...
def random_mask(input_size):
    x1 = 8 #  random.randint(5, 20)
    w1 = 32 # random.randint(20, 34)
    y1 = 8 # random.randint(5, 20)
...
#        pred = pred + 1./2.0*255
       ...
       # for itr in range(FLAGS.batch_size):
        ...
        for itr in range(FLAGS.batch_size):
            utils.save_image(valid_images_masked[itr].astype(np.uint8), FLAGS.logs_dir, name="inp_" + str(85+itr) )
            utils.save_image(pred[itr].astype(np.uint8), FLAGS.logs_dir, name="predz_" + str(85+itr) )     
            utils.save_image(valid_annotations_[itr].astype(np.uint8), FLAGS.logs_dir, name="gt_" + str(85+itr) )        
