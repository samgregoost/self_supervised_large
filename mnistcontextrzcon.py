...
tf.flags.DEFINE_string("logs_dir", "/scratch1/ram095/nips20/logs_contextrzconcat/", "path to logs directory")
...
def inference(images, keep_prob,z,is_train):
    ...
    with tf.variable_scope("inference"):
        ...
        debn = tf.concat([tf.nn.relu(batchnorm(deconv, is_train, name=("debn" + str(decoderLayerNum)))),tf.tile(z,[1,4,4,1])],axis = 3)
        print("#################################")
        print(debn)
  ...
  #  loss = tf.reduce_mean(tf.squared_difference(logits ,annotation ))
    ...
    if FLAGS.mode == "train":
        for itr in xrange(MAX_ITERATION):
            ...
            z_ = np.random.uniform(low=-0.1, high=0.1, size=(FLAGS.batch_size,1,1,512))
