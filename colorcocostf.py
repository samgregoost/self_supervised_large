...
tf.flags.DEFINE_string("logs_dir", "/scratch1/ram095/nips20/logs_colorcocostd/", "path to logs directory")
...
# Convolve.
    logits_smooth = tf.nn.conv2d(tf.abs(logits), gauss_kernel, strides=[1, 1, 1, 1], padding="SAME")
...
#    annotation_weights = (tf.exp(tf.abs(annotation))/tf.exp(1.0))
    loss_ = tf.reduce_mean(tf.reduce_sum(tf.contrib.layers.flatten(tf.abs((annotation - logits))    ),1)) + 0.1*logits_smooth_std
...
#                print(z_)
       ...
       # z_ = m_(z_)
        ...
        print(sess.run(var, feed_dict={image: valid_images, annotation: valid_annotations,z:z_, istrain:False,
                                                    keep_probability: 0.85}    ) )
        print("#######################")
